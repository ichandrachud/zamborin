...
| T8 | tailwind, then site-wide | **MOSTLY NOT A FINDING — my check was wrong, see State** | Claimed: full screen magnifies the whole game instead of showing more of it, and pushes controls off screen. `fit()` pins the logical canvas to the 760x600 house frame whenever MODE is desktop, and `body.focus-mode .game-wrap` in `shared/chrome.css` then stretches that canvas to the viewport, so the drawing scale multiplies: read-outs, pills, buttons and the aeroplane all grow together. Owner-reported on a 2000px-wide window, where the control row was pushed past the foot of the screen and the buttons were simply not there. The wrap is also sized from `100dvh`, which does not always agree with the height actually on screen — the same CSS-dvh trap this project already has a rule about. **Verified against the DEPLOYED `/tailwind/play.js` and the deployed `shared/chrome.css`: both are identical to the prototype's starting point, so this is the shipped behaviour on every game with a fullscreen toggle, not something the boost work introduced.** Fixed in `tailwind-boost` only, as a demonstration: the canvas takes the viewport's real pixels so the UI draws at its own size, and the world scale is anchored to `FRAME_W` so the aeroplane does not grow. Measured at 1280x900: canvas 760x600 to 1280x900, drawing scale 38.5 px/m **unchanged**, wrap sized inline from JS rather than from dvh | **CORRECTED 2026-08-25, and the rollout was CANCELLED after measuring.** My claim had two halves and the load-bearing one was wrong. **Controls are never pushed off screen: measured all 15 games in a same-origin iframe at 1600x1000, every one reports `clipped: false` with zero overflow right and zero overflow bottom.** What the owner saw was Tailwind hiding its control row during flight, which it does deliberately, on a plane that had touched down but was still rolling — normal behaviour, not clipping. The magnification half is real (13 games draw at x1.32 to x1.75 in full screen) but is NOT a defect: for a fixed-board puzzle a bigger board on a bigger screen is the desirable outcome, and the Tailwind fix would only leave a native-size board marooned in empty space. **Tailwind was the genuine case precisely because it is the one scrolling-world game, where showing MORE beats showing BIGGER.** Fixed there, correctly, and not rolled out. One real finding did fall out of the sweep: **stained's full screen does nothing to the game.** Its canvas computes to a hard `width: 760px` and stays 760x600 while the wrap grows to 1140, 1267 and 1368 across three viewport sizes, so the game sits unchanged in a larger empty box while the other fourteen fill theirs. **That pinning IS the S5 fix and must not be casually reverted** — logged, not touched |
